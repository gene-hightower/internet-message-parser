...
// mailtoURI       = "mailto:"i WSP? mailto_to? hfields?
...
uri_list        = "<" first:URI ">" rest:(CFWS? "," CFWS? "<" URI ">")* 
                {
                  return [first, ...rest.map((x: any): URL => x[4])];
                }

list_help       = "List-Help:"i CFWS? uris:uri_list CFWS? CRLF
                {
                  return uris;
                }
list_subscribe  = "List-Subscribe:"i CFWS? uris:uri_list CFWS? CRLF
                {
                  return uris;
                }
list_unsubscribe = "List-Unsubscribe:"i CFWS? uris:uri_list CFWS? CRLF
                {
                  return uris;
                }
...
genericURI      = scheme ":" WSP? hier_part ( "?" query )? ( "#" fragment )?
