{{
import { MIMEVersion, ContentTransferEncoding, ContentType, Parameter } from "./message-types";
...
export const structuredHeaders = [
  ...
  function extractOptional(optional: [string], index: number) {
...
received        = "Received"i WSP* ":" (received_token / CFWS)* ";" date_time CRLF
...
resinfo         = CFWS? ";" meth:methodspec reason:(CFWS reasonspec)?
                  prop:(CFWS propspec+)?
                  {
                    return { method: meth.method,
                             result: meth.result,
                             reason: extractOptional(reason, 1),
                             prop: extractOptional(prop, 1) };
                  }
...
reasonspec      = "reason"i CFWS? "=" CFWS? (token / quoted_string_only)
...
pvalue          = CFWS? ( ( ( local_part? "@" )? domain ) / value )
                  CFWS?
...
arc_info = inst:instance CFWS? ";" payload:authres_payload
         {
           return { instance: Number(inst[5].join("")), payload };
         }
