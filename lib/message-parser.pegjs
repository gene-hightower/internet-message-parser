{{
import { ContentType, Parameter } from "./message-types";
}}
...
from            = "From"i WSP* ":" (mailbox_list / address_list) CRLF
...
content         = "Content-Type"i WSP* ":" CFWS? ty:type CFWS? "/" CFWS? sub:subtype CFWS?
                  params:(";" @parameter)* CRLF
                  {
                    return new ContentType(ty.toLowerCase(),
                                           sub.toLowerCase(),
                                           params ? params.map(function(x:Array<string | Parameter>) { return x[1]; }) : null);
                  }
