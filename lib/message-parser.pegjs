...
encoded_word    = "=?" charset "?" mime_encoding "?" encoded_text "?="

charset         = mime_token

mime_encoding   = mime_token
...
encoded_text    = [\!-\>@-~]+
                  // 1*<Any printable ASCII character other than "?"
                  //   or SPACE>
                  // (but see "Use of encoded-words in message
                  // headers", RFC-2047 section 5)
...
original_recipient =
                   "X-HmXmrOriginalRecipient:"i p:path CRLF
                   {
                     return p;
                   }
...
// <https://datatracker.ietf.org/doc/html/rfc6068#section-2>

//mailtoURI       = "mailto:"i mailto_to? hfields?
//mailto_to       = addr_spec ("," addr_spec)*
//hfields         = "?" hfield ("&" hfield)*
//hfield          = hfname "=" hfvalue
//hfname          = qchar+
//hfvalue         = qchar+

//qchar           = unreserved / pct_encoded / some_delims

//some_delims     = "!" / "$" / "'" / "(" / ")" / "*" / "+" / "," / ";" / ":" / "@"

list_help       = "List-Help:"i CFWS? "<" first:URI ">" rest:(CFWS? "," CFWS? "<" URI ">")* CFWS? CRLF
                {
                  return [first, ...rest.map((x: any): URL => x[4])];
                }

list_subscribe  = "List-Subscribe:"i CFWS? "<" first:URI ">" rest:(CFWS? "," CFWS? "<" URI ">")* CFWS? CRLF
                {
                  return [first, ...rest.map((x: any): URL => x[4])];
                }

list_unsubscribe = "List-Unsubscribe:"i CFWS? "<" first:URI ">" rest:(CFWS? "," CFWS? "<" URI ">")* CFWS? CRLF
                {
                  return [first, ...rest.map((x: any): URL => x[4])];
                }
...
content_location = "Content-Location"i FWS ":" CFWS? URI CFWS? CRLF
...
URI             = uri:(genericURI)
                {
                  return new URL(uri.flat(99).join(""));
                }

genericURI      = scheme ":" hier_part ( "?" query )? ( "#" fragment )?
...
//uri_path        = (path_abempty     // begins with "/" or is empty
...
query           = ( pchar / "/" / "?" / "{" / "|" / "}" )* // <extension to the RFC, allow "{", "|", and "}" -- allowed by whatwg.org

fragment        = ( pchar / "/" / "?" / "{" / "|" / "}" )* // <extension to the RFC, allow "{", "|", and "}" -- allowed by whatwg.org
