...
disposition     = "Content-Disposition"i WSP* ":"
                  disposition_type
                  (";" disposition_parm)*

disposition_type = "inline"i /
                   "attachment"i /
                   extension_token      // values are not case-sensitive
...
filename_parm   = "filename"i "=" value

creation_date_parm = "creation-date" "=" quoted_date_time

modification_date_parm = "modification-date" "=" quoted_date_time

read_date_parm  = "read-date" "=" quoted_date_time

size_parm       = "size" "=" DIGIT+
...
Content_Language = "Content-Language" WSP* ":" CFWS? Language_List CRLF
...
authres_header_field = "Authentication-Results:"i authres_payload

authres_payload = CFWS? authserv_id
                  ( CFWS authres_version )?
                  ( no_result / resinfo+ ) CFWS? CRLF

authserv_id     = token // value
...
reasonspec      = "reason"i CFWS? "=" CFWS? value
...
pvalue          = CFWS? ( value / ( local_part? "@" )? domain )
                  CFWS?
